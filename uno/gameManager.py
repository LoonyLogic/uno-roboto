...
class GameManager:
    ...
    async def user_join(self, guild, category, user):
    ...
    async def deactivate(self):
        for gg in self.guildGames.values():
            await self.close_game(gg)

    async def image_of_cards(self, guild_id, cards, playable=False):
        columns = 8
        card_w = 80
        card_h = 120
        n = len(cards)
        ...
        for i in range(n):
            ...
            if playable:
                if cards[i].reaction > 0:
                    img.paste(Image.open(uno.glob.reaction_image_files[cards[i].reaction]), (card_w * x + 45, card_h * y + 5))
